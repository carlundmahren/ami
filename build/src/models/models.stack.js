...
const constants_1 = require("three/src/constants");
const core_colors_1 = __importDefault(require("../core/core.colors"));
const core_utils_1 = __importDefault(require("../core/core.utils"));
const models_base_1 = __importDefault(require("../models/models.base"));
 ...
 * @module models/stack
 */
class ModelsStack extends models_base_1.default {
    /**
    ...
    constructor() {
        ...
        this._regMatrix = new Matrix4_1.Matrix4();
        this._ijk2LPS = null;
        this._lps2IJK = null;
        this._aabb2LPS = null;
        this._lps2AABB = null;
        ...
        this._spacing = new Vector3_1.Vector3(1, 1, 1);
        this._spacingBetweenSlices = 0;
        this._sliceThickness = 0;
        this._origin = null;
        this._rightHanded = true;
        this._xCosine = new Vector3_1.Vector3(1, 0, 0);
        this._yCosine = new Vector3_1.Vector3(0, 1, 0);
        this._zCosine = new Vector3_1.Vector3(0, 0, 1);
        // convenience vars
    ...
    prepareSegmentation() {
        ...
        for (let i = 0; i < this._frame.length; i++) {
            ...
            mergedFrames[prevIndex].minMax = core_utils_1.default.minMax(mergedFrames[prevIndex]._pixelData);
        }
        ...
        for (let i = 0; i < this._segmentationSegments.length; i++) {
            ...
            if (color === null) {
                ...
                dict[this._segmentationSegments[i].segmentNumber] = core_colors_1.default.cielab2RGB(...color);
            }
        }
        // generate LUTs
        for (let i = 0; i <= max; i++) {
    ...
    prepare() {
        ...
        this._dimensionsIJK = new Vector3_1.Vector3(this._columns, this._rows, this._numberOfFrames);
        this._halfDimensionsIJK = new Vector3_1.Vector3(this._dimensionsIJK.x / 2, this._dimensionsIJK.y / 2, this._dimensionsIJK.z / 2);
        ...
        this._minMax[0] = core_utils_1.default.rescaleSlopeIntercept(this._minMax[0], this._rescaleSlope, this._rescaleIntercept);
        this._minMax[1] = core_utils_1.default.rescaleSlopeIntercept(this._minMax[1], this._rescaleSlope, this._rescaleIntercept);
    ...
    orderFrames() {
        ...
        else if (this._frame[0].instanceNumber !== null &&
            this._frame[1] &&
        ...
        else if (this._frame[0].sopInstanceUID &&
            this._frame[1] &&
    ...
    computeIJK2LPS() {
        // ijk to lps
        this._ijk2LPS = core_utils_1.default.ijk2LPS(this._xCosine, this._yCosine, this._zCosine, this._spacing, this._origin, this._regMatrix);
        // lps 2 ijk
     ...
     */
    computeLPS2AABB() {
        this._aabb2LPS = core_utils_1.default.aabb2LPS(this._xCosine, this._yCosine, this._zCosine, this._origin);
    ...
    _packTo8Bits(channels, frame, textureSize, startVoxel, stopVoxel) {
        ...
        if ((bitsAllocated === 8 && channels === 1) || bitsAllocated === 1) {
            ...
            packed.textureType = constants_1.RGBAFormat;
            packed.data = data;
        }
            ...
            packed.textureType = constants_1.RGBFormat;
            packed.data = data;
        }
        return packed;
    }
        ...
        for (let i = 0; i <= dims.x; i += dims.x) {
            for (let j = 0; j <= dims.y; j += dims.y) {
                for (let k = 0; k <= dims.z; k += dims.z) {
                    let world = new Vector3_1.Vector3(i, j, k).applyMatrix4(this._ijk2LPS);
                    bbox = [
                        Math.min(bbox[0], world.x),
                        Math.max(bbox[1], world.x),
     ...
     */
    AABBox() {
        let world0 = new Vector3_1.Vector3()
            .addScalar(-0.5)
            .applyMatrix4(this._ijk2LPS)
        ...
        let world7 = this._dimensionsIJK
            ...
            .applyMatrix4(this._ijk2LPS)
            .applyMatrix4(this._lps2AABB);
        let minBBox = new Vector3_1.Vector3(Math.abs(world0.x - world7.x), Math.abs(world0.y - world7.y), Math.abs(world0.z - world7.z));
        return minBBox;
    }
    /**
    ...
    _arrayToVector3(array, index) {
        return new Vector3_1.Vector3(array[index], array[index + 1], array[index + 2]);
    }
    _orderFrameOnDimensionIndicesArraySort(a, b) {
        if ('dimensionIndexValues' in a &&
    ...
    static value(stack, coordinate) {
       ...
       Please use core.utils.value instead.`);
        return core_utils_1.default.value(stack, coordinate);
    }
    /**
    ...
    static valueRescaleSlopeIntercept(value, slope, intercept) {
       ...
       Please use core.utils.rescaleSlopeIntercept instead.`);
        return core_utils_1.default.rescaleSlopeIntercept(value, slope, intercept);
    }
    /**
    ...
    static worldToData(stack, worldCoordinates) {
       ...
       Please use core.utils.worldToData instead.`);
        return core_utils_1.default.worldToData(stack._lps2IJK, worldCoordinates);
    }
}
