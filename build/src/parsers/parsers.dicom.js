...
const parsers_volume_1 = __importDefault(require("./parsers.volume"));
const OpenJPEG = __importStar(require("OpenJPEG.js/dist/openJPEG-DynamicMemory-browser.js"));
const decoders_rle_1 = require("../decoders/decoders.rle");
 ...
 */
class ParsersDicom extends parsers_volume_1.default {
    constructor(data, id) {
        super();
        this._id = id;
    ...
    imageOrientation(frameIndex = 0) {
        ...
        if (imageOrientation) {
            ...
            imageOrientation = imageOrientation.split('\\').map(core_utils_1.default.stringToNumber);
        }
    ...
    imagePosition(frameIndex = 0) {
        ...
        if (imagePosition) {
            // make sure we return a number! (not a string!)
            imagePosition = imagePosition.split('\\').map(core_utils_1.default.stringToNumber);
        }
    ...
    instanceNumber(frameIndex = 0) {
        ...
        if (typeof perFrameFunctionnalGroupSequence !== 'undefined') {
            if (perFrameFunctionnalGroupSequence.items[frameIndex].dataSet.elements.x2005140f) {
                let planeOrientationSequence = perFrameFunctionnalGroupSequence.items[frameIndex].dataSet.elements.x2005140f.items[0]
                    .dataSet;
    ...
    pixelSpacing(frameIndex = 0) {
        ...
        if (pixelSpacing) {
            ...
            if (splittedSpacing.length !== 2) {
                ...
                pixelSpacing = splittedSpacing.map(core_utils_1.default.stringToNumber);
            }
        }
        return pixelSpacing;
    }
    ...
    inStackPositionNumber(frameIndex = 0) {
        ...
        if (typeof perFrameFunctionnalGroupSequence !== 'undefined') {
            // NOT A PHILIPS TRICK!
            let philipsPrivateSequence = perFrameFunctionnalGroupSequence.items[frameIndex].dataSet.elements.x00209111.items[0]
                .dataSet;
    ...
    _decodeRLE(frameIndex = 0) {
        ...
        const pixelData = DicomParser.readEncapsulatedPixelDataFromFragments(this._dataSet, this._dataSet.elements.x7fe00010, frameIndex);
        const decoded = (0, decoders_rle_1.RLEDecoder)(imageFrame, pixelData);
...
//# sourceMappingURL=parsers.dicom.js.map