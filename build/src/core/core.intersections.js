...
const core_utils_1 = __importDefault(require("./core.utils"));
const core_validators_1 = __importDefault(require("./core.validators"));
...
class Intersections {
    ...
    static aabbPlane(aabb, plane) {
        ...
        // invert space matrix
        let fromAABB = new Matrix4_1.Matrix4();
        fromAABB.getInverse(aabb.toAABB);
        let t1 = plane.direction.clone().applyMatrix4(aabb.toAABB);
        let t0 = new Vector3_1.Vector3(0, 0, 0).applyMatrix4(aabb.toAABB);
        let planeAABB = this.posdir(plane.position.clone().applyMatrix4(aabb.toAABB), new Vector3_1.Vector3(t1.x - t0.x, t1.y - t0.y, t1.z - t0.z).normalize());
        let bbox = core_utils_1.default.bbox(aabb.center, aabb.halfDimensions);
        let orientation = new Vector3_1.Vector3(new Vector3_1.Vector3(1, 0, 0), new Vector3_1.Vector3(0, 1, 0), new Vector3_1.Vector3(0, 0, 1));
        ...
        //    .'
        //   +
        let ray = this.posdir(new Vector3_1.Vector3(aabb.center.x - aabb.halfDimensions.x, aabb.center.y - aabb.halfDimensions.y, aabb.center.z - aabb.halfDimensions.z), orientation.x);
        ...
        //           +
        //
        let ray2 = this.posdir(new Vector3_1.Vector3(aabb.center.x + aabb.halfDimensions.x, aabb.center.y + aabb.halfDimensions.y, aabb.center.z + aabb.halfDimensions.z), orientation.x);
        ...
        //             .'
        //           +'
        let ray3 = this.posdir(new Vector3_1.Vector3(aabb.center.x + aabb.halfDimensions.x, aabb.center.y - aabb.halfDimensions.y, aabb.center.z - aabb.halfDimensions.z), orientation.y);
        ...
        //
        //
        let ray4 = this.posdir(new Vector3_1.Vector3(aabb.center.x - aabb.halfDimensions.x, aabb.center.y + aabb.halfDimensions.y, aabb.center.z - aabb.halfDimensions.z), orientation.x);
        ...
        //   |
        //   +-------+
        let ray5 = this.posdir(new Vector3_1.Vector3(aabb.center.x - aabb.halfDimensions.x, aabb.center.y - aabb.halfDimensions.y, aabb.center.z + aabb.halfDimensions.z), orientation.x);
    ...
    static rayPlane(ray, plane) {
        ...
        if (ray.direction.dot(plane.direction) !== 0) {
            ...
            //
            let t = (plane.direction.x * (plane.position.x - ray.position.x) +
            ...
            let intersection = new Vector3_1.Vector3(ray.position.x + t * ray.direction.x, ray.position.y + t * ray.direction.y, ray.position.z + t * ray.direction.z);
            return intersection;
        }
        return null;
    }
    ...
    static rayBox(ray, box) {
        ...
        let intersections = [];
        let bbox = core_utils_1.default.bbox(box.center, box.halfDimensions);
        // window.console.log(bbox);
        // X min
        let plane = this.posdir(new Vector3_1.Vector3(bbox.min.x, box.center.y, box.center.z), new Vector3_1.Vector3(-1, 0, 0));
        this.rayPlaneInBBox(ray, plane, bbox, intersections);
        // X max
        plane = this.posdir(new Vector3_1.Vector3(bbox.max.x, box.center.y, box.center.z), new Vector3_1.Vector3(1, 0, 0));
        this.rayPlaneInBBox(ray, plane, bbox, intersections);
        // Y min
        plane = this.posdir(new Vector3_1.Vector3(box.center.x, bbox.min.y, box.center.z), new Vector3_1.Vector3(0, -1, 0));
        this.rayPlaneInBBox(ray, plane, bbox, intersections);
        // Y max
        plane = this.posdir(new Vector3_1.Vector3(box.center.x, bbox.max.y, box.center.z), new Vector3_1.Vector3(0, 1, 0));
        this.rayPlaneInBBox(ray, plane, bbox, intersections);
        // Z min
        plane = this.posdir(new Vector3_1.Vector3(box.center.x, box.center.y, bbox.min.z), new Vector3_1.Vector3(0, 0, -1));
        this.rayPlaneInBBox(ray, plane, bbox, intersections);
        // Z max
        plane = this.posdir(new Vector3_1.Vector3(box.center.x, box.center.y, bbox.max.z), new Vector3_1.Vector3(0, 0, 1));
        this.rayPlaneInBBox(ray, plane, bbox, intersections);
        return intersections;
    }
    /**
    ...
    static validatePlane(plane) {
        ...
        if (!core_validators_1.default.vector3(plane.position)) {
            window.console.log('Invalid plane.position.');
            window.console.log(plane.position);
            return false;
        }
        if (!core_validators_1.default.vector3(plane.direction)) {
    ...
    static validateAabb(aabb) {
        ...
        if (!core_validators_1.default.matrix4(aabb.toAABB)) {
            window.console.log('Invalid aabb.toAABB: ');
            window.console.log(aabb.toAABB);
            return false;
        }
        if (!core_validators_1.default.vector3(aabb.center)) {
            window.console.log('Invalid aabb.center.');
            window.console.log(aabb.center);
            return false;
        }
        if (!(core_validators_1.default.vector3(aabb.halfDimensions) &&
...
//# sourceMappingURL=core.intersections.js.map