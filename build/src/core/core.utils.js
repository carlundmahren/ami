...
const Box3_1 = require("three/src/math/Box3");
const Raycaster_1 = require("three/src/core/Raycaster");
const Triangle_1 = require("three/src/math/Triangle");
...
class CoreUtils {
    ...
    static bbox(center, halfDimensions) {
        ...
        if (!(core_validators_1.default.vector3(center) && core_validators_1.default.vector3(halfDimensions))) {
     ...
     * @return {*}
     */
    static ijk2LPS(xCos, yCos, zCos, spacing, origin, registrationMatrix = new Matrix4_1.Matrix4()) {
        const ijk2LPS = new Matrix4_1.Matrix4();
    ...
    static aabb2LPS(xCos, yCos, zCos, origin) {
        const aabb2LPS = new Matrix4_1.Matrix4();
    ...
    static worldToData(lps2IJK, worldCoordinates) {
        let dataCoordinate = new Vector3_1.Vector3().copy(worldCoordinates).applyMatrix4(lps2IJK);
    ...
    static centerOfMass(points) {
        let centerOfMass = new Vector3_1.Vector3(0, 0, 0);
    ...
    static orderIntersections(points, direction) {
        ...
        let referenceDirection = new Vector3_1.Vector3(points[0].x - reference.x, points[0].y - reference.y, points[0].z - reference.z).normalize();
        let base = new Vector3_1.Vector3(0, 0, 0).crossVectors(referenceDirection, direction).normalize();
        let orderedpoints = [];
        // other lines // if inter, return location + angle
        for (let j = 0; j < points.length; j++) {
            let point = new Vector3_1.Vector3(points[j].x, points[j].y, points[j].z);
            point.direction = new Vector3_1.Vector3(points[j].x - reference.x, points[j].y - reference.y, points[j].z - reference.z).normalize();
    ...
    static getRoI(mesh, camera, stack) {
        ...
        const bbox = new Box3_1.Box3().setFromObject(mesh);
    ...
    static getGeometryArea(geometry) {
        ...
        geometry.faces.forEach(function (elem) {
            area += new Triangle_1.Triangle(vertices[elem.a], vertices[elem.b], vertices[elem.c]).getArea();
        });
        return area;
    }
